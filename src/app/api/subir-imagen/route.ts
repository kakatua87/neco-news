...
export async function POST(request: Request) {
  try {
    ...
    const noticiaId = formData.get("noticia_id");

    if (!(file instanceof File)) {
      return NextResponse.json({ ok: false, error: "Falta el archivo" }, { status: 400 });
    }
    if (!noticiaId) {
      return NextResponse.json({ ok: false, error: "Falta noticia_id" }, { status: 400 });
    }
    ...
    if (file.size > MAX_BYTES) {
      return NextResponse.json(
        { ok: false, error: "La imagen pesa demasiado (máx 8MB)." },
        { status: 400 }
      );
    }
    ...
    const path = `noticias/${noticiaId}-manual-${Date.now()}.${extension}`;
    ...
    console.error("Catch error in POST subir-imagen:", err);
